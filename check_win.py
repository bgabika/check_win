...
# -*- coding: utf-8 -*-
# ---------------------------------------------------------------
# COREX Windows host check plugin for Icinga 2
# Copyright (C) 2019-2022, Gabor Borsos <bg@corex.bg>
# 
# v1.13 built on 2022.12.24.
...
class CheckWin:
    ...
    def parse_args(self):
        parser = argparse.ArgumentParser(
            ...
            PLUGIN DESCRIPTION: COREX True agentless Windows check plugin for ICINGA 2.
        ...
        api_connect_opt.add_argument('--sshkey', type=str, required=False, help="ssh key file")


        check_win_opt = parser.add_argument_group('check arguments', 'cpu, disk-health, disk-usage, memory, network, procs, swap, user-logged-on, user-count, winfo')
        
        check_win_opt.add_argument("--subcommand",
                                        choices=(
                                            'cpu', 'disk-health', 'disk-usage', 'memory', 'network', 'procs', 'swap', 'user-logged-on', 'user-count', 'winfo'),
                                        required=True,
        ...
        if (self.options.subcommand == "cpu" or self.options.subcommand == "disk-usage" or self.options.subcommand == "memory" \
